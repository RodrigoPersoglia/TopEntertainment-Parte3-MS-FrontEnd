...
const CargarVarios = () => {
    ...
    .then(data => {
        ...
        for (var i = 0; i < 4; i++) {
            ...
            subcontainer1.innerHTML+=CardVarios(list[i].nombreProducto,list[i].precio.toLocaleString('fr-FR', {style: 'currency',currency: 'USD', minimumFractionDigits: 2}),list[i].imagenes[0])
            subcontainer2.innerHTML+=CardVarios(list[x].nombreProducto,list[x].precio.toLocaleString('fr-FR', {style: 'currency',currency: 'USD', minimumFractionDigits: 2}),list[x].imagenes[0])
            subcontainer3.innerHTML+=CardVarios(list[y].nombreProducto,list[y].precio.toLocaleString('fr-FR', {style: 'currency',currency: 'USD', minimumFractionDigits: 2}),list[y].imagenes[0])
         }
    });
